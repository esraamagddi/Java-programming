...
import gov.iti.day6.lab.domain.Country;
...
public class InMemoryWorldDao implements WorldDao {
    ...
    public static void main(String args[]) {
        ...
        //list 
        //data ===> inside collection ===> process wrapper into stream ==> mainapulate a copy from original collection##
        //stream ===> pipe line intermadiate operation (never execute if there is not terminal operation) - terminal operation 
        //array ==>Array.stream(employee)
        //
        ...
        //min max average===>class<? extends Number>===> max(implemenation)
        //max((emp1,emp2)->(emp1->id)>(emp2->id))
        //max((emp1,emp2)->(emp1->name.compare(emp2.name))
        //after finish the operation collection=> //stream => collection // mutable reduction collect //toarray
        //Employee ===>id ,name , gpa ====>reduce(lambda experession(implementation))

        //datatype collectin = collection.stream().collect;

        //empList.stream().forEach(e -> e.salaryIncrement(10.0)).=====>peek
        //empList.stream().peek((e -> e.salaryIncrement(10.0))).filter(e->e.salary>5000).collect(Collector.tolist);


        //map ===> employee //id ====>employee.getId()===>instead using list of object employee type object employee using list of employeeid type integer 
        //map datatype inside stream to another datatype size list 50 employee ??num of item in the list are the same 50 id 
        ///map didn't change the size of list if there 50 element will result 50##
        //stream use only refrence ==> IntStream // doubleStream

        //IntStream.of(values).forEach(value ->System.out.printf("%d ", value)).max(); not correct  
        //int x =IntStream.of(values).peek(value ->System.out.printf("%d ", value)).max(); correct   




        Map<String, Country> counrt=dao.getCountries;//key : value 
        List<Country> countries = dao.getCountries().values().stream().collect(Collectors.toList());//int x= 5  Integer {x=5: methods} 
        ...
        for (City city : highestPopulatedCities) {
        if (city != null) {
        System.out.println("City: " + city.getName() + ", Population: " + city.getPopulation());
        } else {
        System.out.println("No city found for the corresponding country.");
        }
        }
        List<City> highestPopulatedCities2 = countries.stream().map(c->c.getCities().stream.max(Comparator.comparingInt((c1,c2)->(c1.getPopulation()>c2.getPopulation))).collect(Collector.tolist);//(c1,c2)->(c1.getPopulation()>c2.getPopulation)
